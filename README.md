...
There have been many capacitor footprints added to the edge of the ISA bus, on the left of the mainboard. These do not all need to be populated, but are instead intended for termination of signal reflections, in case there are some issues occuring. Specifically, active high signals may be sensitive to noise generated from reflections. The best example is the RESETDRV signal on the ISA bus. If there is too much noise on such a signal it may cause certain I/O ICs such as UARTs to operate erroniously. For this reason the onboard UART RESET input has been purposely grounded. Use a 2.2nF capacitor for RESETDRV at least.  

I want to caution against soldering in all of these parts, that is not the purpose of these footprints. Start with the RESETDRV capacitor, and then first get the system up and running, and then do scope measurements with your target CPU clock frequency to see if any interference or ringing stands out. It's easy to test by inserting a capacitor into the footprint holes and measure/test the resulting effect on a certain signal if it looks particularly bad on a scope. Don't solder in all these capacitors right away because I can't predict how this will impact the entire system operation by changing all the impedances. These footprints are only intended to solve any noise, ringing or extreme interference on signals, and is not a default configuration to include all of these without a clear reason leading to doing that. In fact, less capacitors needed would be better overall for the system operation.
...
Additionally, it will contain a lot less components than this project. And I will probably experiment with replacing the bus controller ICs with CPLD logic based designs as I have done with the 286 CPU. This work also depends on how much documentation about the 8088 cycle operation I will find, if the diagrams etc are enough information for me to be able to work with. Another upgrade will be that a USB mouse connection will be included based on the usb to serial project by limeprogramming. So the 12V level shifter and COM1 port will be replaced with a USB header for a USB mouse and there will be a RP2040 inserted into the board using an angled connector. I will publish more about all the future design changes later.  
